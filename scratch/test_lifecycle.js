...
async function runLifecycleVerification() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (convertedAgain.id !== project.id) {
      throw new Error(`Idempotency failure! Blueprint ${bp.id} converted into two different projects: ${project.id} and ${convertedAgain.id}`);
    }
  ...
  for (const pid of createdProjectIds) {
    const run = await createValidationRun({
      ...
      issues: [
        { id: `iss_1_${pid}`, title: "Missing open-source license file", description: "Root directory lacks LICENSE", severity: "high", fixText: "Add MIT license" }
      ],
      roadmap: [],
    });
